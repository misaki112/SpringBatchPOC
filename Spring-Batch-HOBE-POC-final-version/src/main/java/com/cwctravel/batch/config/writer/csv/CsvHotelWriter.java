...
/**
 * CsvHotelWriter is a JdbcBatchItemWriter writing hotel data from csv file
 * to destination MySQL database with toleration to dead lock and lock wait timeout errors.
 * @author chris.nie
 *
 */
public class CsvHotelWriter extends JdbcBatchItemWriter<HotelAndRateCombination> {
	 ...
	 */
	@Override
	public void write(List<? extends HotelAndRateCombination> items) throws Exception {
		int count = 0;
		int maxTries = 20;
		while(true) {
			try {
